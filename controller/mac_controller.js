const SongModel = require('../models/music_model');
...
searchSongs: (req, res) => {
    ...
    SongModel.searchSongs(query, (err, songs) => {
        if (err) {
            res.status(500).send(err);
        } else {
            res.render('search', { songs, query }); // Render the search page with results
        }
    });
},
...
getSuggestions: (req, res) => {
    ...
    SongModel.getSuggestions(query, (err, songs) => {
        if (err) {
            res.status(500).send(err);
        } else {
            res.json(songs); // Send the matching songs as JSON
        }
    });
},
